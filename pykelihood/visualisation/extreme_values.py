...
from pykelihood.visualisation.utils import get_quantiles_and_confidence_intervals, \
...
def qq_plot_gpd(data: pd.DataFrame, gpd_fit: Distribution, path_to_figure: str,
    ...
    if len(gpd_fit.flattened_params) != len(gpd_fit.params):
        id_obs = False
    theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals(gpd_fit, data,
                                                                                              ci_confidence) if id_obs else get_quantiles_and_confidence_intervals_uniform_scale(
        gpd_fit, data,
        ci_confidence)
    n = len(data)
    text_title = ''
    ...
    if id_obs:
        plt.fill_betweenx(y=empirical, x1=lower_bound, x2=upper_bound, alpha=0.2, color='navy')
    else:
    ...
    plt.clf()
    to_concat = pd.DataFrame([theoretical, lower_bound, upper_bound], columns=empirical.index,
                             index=['theoretical', 'lower_bound', 'upper_bound']).T
    return pd.concat([empirical.rename('realized'), to_concat], axis=1)


def qq_plot_gev(data: pd.DataFrame, gev_fit: Distribution, path_to_figure: str,
                ci_confidence=0.99, figure_name='qq_plot_gev'):
    id_obs = True
    if len(gev_fit.flattened_params) != len(gev_fit.params):
        id_obs = False
    theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals(gev_fit, data,
                                                                                              ci_confidence) if id_obs else get_quantiles_and_confidence_intervals_uniform_scale(
        gev_fit, data,
        ci_confidence)
    n = len(data)
...
def consecutive_days_above_value_plot(data: pd.DataFrame, value: float, path_to_figure: str):
...
def consecutive_days_under_value_plot(data: pd.DataFrame, value: float, path_to_figure: str):
...
def qq_plot_exponential(data: Union[pd.Series, np.array],
                        exp_fit: Distribution,
                        path_to_figure: str, ci_confidence=0.99,
                        figure_name='qq_plot_exponential'):
    id_obs = True
    if len(exp_fit.flattened_params) != len(exp_fit.params):
        id_obs = False
    theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals(exp_fit, data,
                                                                                              ci_confidence) if id_obs else get_quantiles_and_confidence_intervals_uniform_scale(
        exp_fit, data,
        ci_confidence)
    plt.scatter(theoretical, empirical, s=5, marker='x', color="navy")
    ...
    plt.ylabel(r"Exponential with parameter $\lambda$" + f"= {round(exp_fit.rate(), 2)}")
    plt.savefig(f"{path_to_figure}/{figure_name}.png")
    plt.clf()
    to_concat = pd.DataFrame([theoretical, lower_bound, upper_bound], columns=empirical.index,
                             index=['theoretical', 'lower_bound', 'upper_bound']).T
...
def extremogram_plot(data: pd.DataFrame, h_range: Union[List, np.array], path_to_figure: str, compare_to_hawkes=False,
    ...
    indices_exceedances = [int(round(e, 0)) for e in list(data_extremogram["days_since_start"])]
    ...
    exp_fit = Exponential.fit(data_extremogram['iat'], loc=0.)
    exceedances_pp = []
    for i in range(1000):
    ...
    to_return = pd.concat([extremogram_realized.rename('realized'),
                           mean_pp.rename('mean_pp'),
                           pd.DataFrame([quantile_inf_pp, quantile_sup_pp], index=['pp_lb', 'pp_ub'], columns=h_range).T],
                          axis=1)
    ...
    if compare_to_hawkes:
        if UEHP is not None:
            ...
            alpha = 0
            theta = 0
        hawkes_fit = Exponential.fit(data_extremogram["iat"], x0=(0., mu, alpha, theta),
        ...
        to_return = pd.concat([to_return, mean_hp.rename('mean_hp'), pd.DataFrame([quantile_inf_hp, quantile_sup_hp], index=['hp_lb', 'hp_ub'],
                                                                                  columns=h_range).T])
...
def mean_cluster_size(data: pd.DataFrame, block_sizes: Union[List, np.array],
                      path_to_figure: str, compare_to_hawkes=False,
    ...
    # Simulated
    exp_fit = Exponential.fit(data_extremal_index['iat'], loc=0.)
    exceedances_pp = []
    for i in range(1000):
    ...
    to_return = pd.concat([empirical_mean_cluster_size.rename('realized'),
                           mean_pp.rename('mean_pp'),
                           pd.DataFrame([quantile_inf_pp, quantile_sup_pp], index=['pp_lb', 'pp_ub'],columns=x).T], axis=1)
    ...
    if compare_to_hawkes:
        if UEHP is not None:
            ...
            alpha = 0
            theta = 0
        hawkes_fit = Exponential.fit(data_extremal_index["iat"], x0=(0., mu, alpha, theta),
                                     rate=kernels.hawkes_with_exp_kernel(
        ...
        to_return = pd.concat([to_return, mean_hp.rename('mean_hp'), pd.DataFrame([quantile_inf_hp, quantile_sup_hp],
                                                                                  columns=x, index=['hp_lb', 'hp_ub']).T])
...
def cum_number_exceedances(data: pd.DataFrame,
    ...
    # Empirical
    realized = data.set_index("time").assign(bool=lambda x: (x["data"] >= x["threshold"]).astype(int))["bool"]
    realized = realized[realized == True].cumsum().reset_index().set_index("bool")["time"]
    ...
    # Simulated Poisson process
    exp_fit = Exponential.fit(data_gpd['iat'], loc=0.)
    simulations = []
    for _ in range(1000):
        simulations.append(pd.DataFrame(np.cumsum(exp_fit.rvs(len(data_gpd))), columns=[_]))
    ...
    x1_pp = pd.to_datetime(length_total_period * quantile_inf_pp,
                           unit="d", origin=origin)
    x2_pp = pd.to_datetime(length_total_period * quantile_sup_pp,
                           unit="d", origin=origin)
    ...
    if compare_to_hawkes:
        if UEHP is not None:
            ...
            alpha = 0
            theta = 0
        hawkes_fit = Exponential.fit(data_gpd["iat"], x0=(0., mu, alpha, theta),
                                     rate=kernels.hawkes_with_exp_kernel(np.array(data_gpd["time"])))
        simulations_hp = []
        for _ in range(1000):
            simulations_hp.append(pd.DataFrame(np.cumsum(hawkes_fit.rvs(len(data_gpd))), columns=[_]))
        ...
        x1 = pd.to_datetime(length_total_period * quantile_inf_hp,
                            unit="d", origin=origin)
        x2 = pd.to_datetime(length_total_period * quantile_sup_hp,
                            unit="d", origin=origin)
        mean_hp_.index = pd.to_datetime(length_total_period * mean_hp,
                                        unit="d", origin=origin)
...
def Kgaps_diagnostic_plots(range_K: Union[List, np.array],
    ...
    for K in Ks:
        ...
        x = (bins[:-1] + bins[1:]) / 2
        theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals(
        ...
        plt.fill_betweenx(y=empirical, x1=lower_bound, x2=upper_bound, alpha=0.5, color='navy')
        ax1.legend()
