...
def get_quantiles_and_confidence_intervals_uniform_scale(fit: Distribution,
    ...
    for level in levels:
        ...
        min_max.append(CI)
    min_max = pd.DataFrame(min_max, columns=['lower_bound', 'upper_bound'], index=levels)
...
def get_quantiles_and_confidence_intervals(fit: Distribution,
                                           data: Union[pd.DataFrame, np.array, pd.Series],
                                           ci_confidence=0.99):
    ll = Likelihood(fit, data, inference_confidence=ci_confidence)
    min_max = []
...
def qq_plot_uniform_scale(fit: Distribution,
            data: Union[pd.DataFrame, np.array, pd.Series],
            path_to_figure: str,
            ci_confidence=0.99,
            figure_name = 'qq_plot'):
    theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals_uniform_scale(fit, data,
                                                                                              ci_confidence)
    n = len(data)
...
def qq_plot(fit: Distribution,
            data: Union[pd.DataFrame, np.array, pd.Series],
            path_to_figure: str,
            ci_confidence=0.99,
            figure_name = 'qq_plot'):
    theoretical, empirical, lower_bound, upper_bound = get_quantiles_and_confidence_intervals(fit, data,
                                                                                              ci_confidence)
    import matplotlib.pyplot as plt
    n = len(data)
    plt.scatter(theoretical, empirical, s=5, color="navy")
